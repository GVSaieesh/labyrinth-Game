...
class HintNode:
    """Node for A* hint-finding algorithm"""
...
class LabyrinthGame:
    ...
    def determine_hint_difficulty(self):
        ...
        # Determine base difficulty by level
        base_difficulty = "Easy" if level_num <= 2 else "Medium" if level_num <= 4 else "Hard"
        
        # If stuck, provide easier hints
        if current_node.stuck_count > 2:
        ...
        # If making good progress, make hints more challenging
        if progress_percentage > 70:
            # Higher difficulty by one level
        ...
        # Apply dynamic difficulty based on puzzle attempts
        if current_node.puzzle_attempts > 3:
            # Lower difficulty after multiple failed attempts
            if base_difficulty == "Hard":
                return "Medium"
            elif base_difficulty == "Medium":
                return "Easy"
    ...
    def generate_hints(self, state):
        ...
        # Get difficulty based on player progress
        difficulty = self.determine_hint_difficulty()
        
        # Puzzle hints - adapt based on level
        if node.puzzle and not node.puzzle.get("solved", False):
            # Different hint categories by level
            if "Ancient Entrance" in location:  # Level 1
            ...
            elif "Crystal Caverns" in location:  # Level 2
            ...
            elif "Shadow Maze" in location:  # Level 3
            ...
            elif "Elemental Chambers" in location:  # Level 4
            ...
            elif "Time-Lost Library" in location:  # Level 5
                ...
                # Default hints if we can't determine the level
                general = f"Focus on the puzzle in {location}."
                specific = node.puzzle.get("hint", "Look at the options.")
                very_specific = f"The answer is {node.puzzle['options'][node.puzzle['correct_option']]}."
            
            # Assign hint based on difficulty
            if difficulty == "Easy":
        ...
        # Required items hints - adapt based on level and progress
        if node.locked:
            for item in node.required_items:
                if item not in inventory:
                    for ploc, pnode in self.labyrinth.items():
                        if item in pnode.items:
                            if level_num <= 3:  # More direct for early levels
                                hint = f"You need to find the {item} in the {ploc}."
                            else:  # More cryptic for later levels
                                hint = f"A key from {ploc} will unlock this path."
                            
                            hints.append((hint, 2))
        
        # Boss hints - more detailed in early levels, vague in later levels
        ...
        # Navigation hints - more detailed early, vague later
        ...
        for hint, cost in hints:
            ...
            # Penalty increases with each repetition
    ...
    def calculate_heuristic(self, state):
        ...
        if node.locked and any(item not in inventory for item in node.required_items):
            urgency += 15
        if node.boss and not node.boss.get("defeated", False):
            urgency += 20 * game_stage  # Higher priority in later stages
        
        # Time factor - prioritize helpful hints as player spends more turns
        time_factor = min(20, player_turns / 5)
        
        # Final calculation
        return (puzzle_value + urgency + stuck_penalty + time_factor) - inventory_progress + redundancy_factor

    def a_star_search(self, initial_state, goal_test, heuristic):
        """A* search for finding optimal hints"""
        frontier = []
        explored = set()
    ...
    def check_node_first_visit(self):
        """Check if this is first visit to the current node"""
        current = self.labyrinth[self.current_location]
